    ...
    <div className="min-h-screen bg-black text-slate-200 selection:bg-emerald-900 selection:text-white">
      ...
      <section className="relative overflow-hidden pt-16 pb-10 sm:pt-32 sm:pb-24">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
          ...
          >
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-600 mb-4 sm:mb-6 pb-2">
      ...
      {/* CV Knowledge Section (Now at the top) */}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
          ...
          >
            <motion.div variants={fadeInUp}>
              ...
              <h2 className="text-2xl sm:text-4xl font-bold text-white mb-6 sm:mb-8">Computer Vision Toolkit</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
                {[
                  { title: "Object Detection", desc: "I implemented YOLO architectures and fine-tuned models to locate and classify entities in dynamic environments. I achieved high-accuracy and low-latency detection by optimizing the network for our specific use cases.", color: "border-emerald-500/20 bg-emerald-500/5 text-emerald-300" },
                  { title: "Image Segmentation", desc: "I developed segmentation models to generate pixel-perfect boundaries, giving our robots precise spatial awareness. I used this to successfully identify drivable areas, detect obstacles, and create accurate depth maps for safe navigation.", color: "border-teal-500/20 bg-teal-500/5 text-teal-300" },
                  { title: "DEEPSORT Tracking", desc: "I integrated DEEPSORT to enable persistent identity tracking across complex video frames. I ensured our system could reliably maintain object states through occlusions and assign unique tracking IDs in real-world scenarios.", color: "border-green-500/20 bg-green-500/5 text-green-300" },
                  { title: "Multi-Camera Systems", desc: "I built sensor fusion pipelines that integrated LIDAR and Camera data. I managed the extrinsic calibration, synchronized multiple sensor streams, and successfully mapped 2D RGB data onto rich 3D point clouds.", color: "border-cyan-500/20 bg-cyan-500/5 text-cyan-300" },
                  { title: "Edge Deployment", desc: "I deployed our real-time computer vision models directly to edge devices like the Jetson Nano and Raspberry Pi. I leveraged GPUs, TensorRT, and CUDA to achieve maximum hardware acceleration and significantly boost our FPS.", color: "border-blue-500/20 bg-blue-500/5 text-blue-300" }
                ].map((item, i) => (
                  <motion.div 
                    key={i} 
                    variants={fadeInUp}
                    whileHover={{ scale: 1.02 }}
                    className={`p-5 sm:p-6 rounded-xl border ${item.color} backdrop-blur-sm transition-all h-full`}
                  >
                ...
                <motion.div 
                  variants={fadeInUp}
                  whileHover={{ scale: 1.02 }}
                  className="p-5 sm:p-6 rounded-xl border border-purple-500/20 bg-purple-500/5 backdrop-blur-sm transition-all h-full flex flex-col justify-center"
                >
                  <h3 className="font-semibold text-lg sm:text-xl mb-4 text-purple-300">Open Source Projects</h3>
                  <div className="flex flex-col gap-4">
                    ...
                    >
                      <ChevronRight className="w-5 h-5 text-purple-400 shrink-0 mt-0.5 group-hover:translate-x-1 transition-transform" />
                      <div>
                        <p className="text-purple-300 font-medium text-sm sm:text-base">Object Detection for Drones</p>
                        <p className="text-slate-500 text-xs mt-1">View GitHub Repository</p>
                      </div>
                    </a>
                    <a 
                      href="https://github.com/Smeet-Dedhia/DeepLearning-Lab-Projects"
                      target="_blank"
                      rel="noreferrer"
                      className="group flex items-start gap-3"
                    >
                      <ChevronRight className="w-5 h-5 text-purple-400 shrink-0 mt-0.5 group-hover:translate-x-1 transition-transform" />
                      <div>
                        <p className="text-purple-300 font-medium text-sm sm:text-base">CV & Deep Learning Lab</p>
                        <p className="text-slate-500 text-xs mt-1">View GitHub Repository</p>
                      </div>
                    </a>
                  </div>
          ...
          >
            <div className="order-1 lg:order-2">
              <motion.div variants={fadeInUp} className="mb-6 sm:mb-8">
                ...
                <p className="text-cyan-500 text-sm sm:text-base font-medium mb-1">AI Systems Engineer <span className="text-slate-500 mx-2">|</span> Manipal, India</p>
                <p className="text-xs sm:text-sm text-slate-400">Jul 2022 - Sep 2023</p>
              </motion.div>
            ...
            <motion.div variants={fadeInUp} className="flex flex-col gap-4 sm:gap-6 w-full order-2 lg:order-1">

        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              {/* Data & Cloud Stack (Right) */}
              <motion.div variants={fadeInUp} className="flex flex-col gap-6">
                {/* AWS Services Stack */}
                <div className="flex flex-col items-center p-5 sm:p-6 rounded-2xl border border-slate-800 bg-slate-900/50 backdrop-blur-sm flex-1">
                  <h3 className="text-slate-300 font-semibold mb-4 flex items-center gap-2"><Database className="w-4 h-4 text-blue-400" /> AWS Services</h3>
                  <div className="flex flex-wrap justify-center gap-2 sm:gap-3 w-full">
                    {[
                      { name: "S3", icon: <Database className="w-6 h-6 text-emerald-500" /> },
                      { name: "Redshift", icon: <Database className="w-6 h-6 text-indigo-400" /> },
                      { name: "Athena", icon: <Zap className="w-6 h-6 text-cyan-400" /> },
                      { name: "CloudFormation", icon: <Layers className="w-6 h-6 text-rose-500" /> }
                    ].map(tech => (
                      <div key={tech.name} className="flex flex-col items-center gap-2 group">
                        <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center p-2 group-hover:bg-slate-700 transition-colors group-hover:scale-110 duration-300">
                          {tech.icon ? tech.icon : <img src={tech.url} alt={tech.name} className="w-full h-full object-contain" />}
                        </div>
                        <span className="text-[11px] sm:text-xs text-slate-500 font-medium group-hover:text-slate-300 transition-colors whitespace-nowrap">{tech.name === "CloudFormation" ? "CloudForm" : tech.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Big Data Stack */}
                <div className="flex flex-col items-center p-5 sm:p-6 rounded-2xl border border-slate-800 bg-slate-900/50 backdrop-blur-sm flex-1">
                  <h3 className="text-slate-300 font-semibold mb-4 flex items-center gap-2"><Server className="w-4 h-4 text-amber-400" /> Big Data Stack</h3>
                  <div className="flex flex-wrap justify-center gap-2 sm:gap-3 w-full">
                    {[
                      { name: "Spark", url: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/apachespark/apachespark-original.svg" },
                      { name: "Hadoop", url: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/hadoop/hadoop-original.svg" },
                      { name: "HDFS", icon: <Server className="w-6 h-6 text-yellow-500" /> },
                      { name: "PySpark", icon: <Code2 className="w-6 h-6 text-blue-400" /> }
                    ].map(tech => (
                      <div key={tech.name} className="flex flex-col items-center gap-2 group">
                        <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center p-2 group-hover:bg-slate-700 transition-colors group-hover:scale-110 duration-300">
                          {tech.icon ? tech.icon : <img src={tech.url} alt={tech.name} className="w-full h-full object-contain" />}
                        </div>
                        <span className="text-[11px] sm:text-xs text-slate-500 font-medium group-hover:text-slate-300 transition-colors whitespace-nowrap">{tech.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
